document.addEventListener('DOMContentLoaded', () => {
    ...
    (function(index) {
        ...
        squares[i].onclick = function () {
            ...
            if(squares[index+7].classList.contains('taken')) {
                ...
                // if the square below yours is not taken, you can't go here
    ...
    function checkBoard() {
        ...
        //now take the four values in the winning arrays and plug them into the squares
        for (let y = 0; y < winningArrays.length; y++) {
            ...
            //now check those arrays to see if they have a class of player one 
            if(square1.classList.contains('player-one') && 
            square2.classList.contains('player-one') &&
            square3.classList.contains('player-one') &&
            square4.classList.contains('player-one')) {
                //if they do, player one is passed as the winner
                result.innerHTML = 'player one WINR! WOO!'
            }

            //now check those arrays to see if they have a class of player two
            else if (square1.classList.contains('player-two') && 
            square2.classList.contains('player-two') &&
            square3.classList.contains('player-two') &&
            square4.classList.contains('player-two')) {
                //if they do, player one is passed as the winner
                result.innerHTML = 'player two WINR! WOO!'
            }
